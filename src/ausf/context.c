...
ausf_ue_t *ausf_ue_add(char *suci)
{
    ...
    tls_server_wrapper_t *tls_server = ogs_malloc(sizeof *tls_server);
    memset(tls_server, 0, sizeof *tls_server);
    init_tls_server(tls_server);
    ausf_ue->tls_server = tls_server; 
    memset(&e, 0, sizeof(e));
    e.ausf_ue = ausf_ue;
...
void ausf_ue_remove(ausf_ue_t *ausf_ue)
{
    ...
    ogs_assert(ausf_ue->tls_server);
    ogs_free(ausf_ue->tls_server);
