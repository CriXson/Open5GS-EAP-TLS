...
#include "../../lib/sbi/openapi/model/eap_session.h"
#include "../../lib/eaptls/eap.h" 
        ...
        ogs_sbi_stream_t *stream, ogs_sbi_message_t *recvmsg)
{
    ...
    //OpenAPI_eap_session_t EAP_SESSION;
    //char eap_payload_base64[OGS_KEYSTRLEN(1409)]; // OGS_NAS_MAX_EAP_MESSGE_LEN=1409
    ...
    if (AuthenticationInfoResult->auth_type ==
        ...
        UeAuthenticationCtx.auth_type = ausf_ue->auth_type; 
        //eap payload tls start message has no tls data 
        eap_packet_t packet;
        memset(&packet, 0, sizeof(packet));
        uint8_t flag = tls_start_flag();
        create_request_packet(&packet, flag, NULL, 0, 0);

        // packet to hex and then base64
        int eap_len = 6;
        int payload_len = 0;
        uint8_t eap_bytes[eap_len]; 
        memset(eap_bytes, 0, eap_len);
        packet2stringHex(eap_bytes ,&packet, payload_len);
        ogs_info("EAP paket string hex: %s", eap_bytes);
        size_t base64Len = ogs_base64_encode_len(eap_len);
        char *eap_base64 = ogs_calloc(1, base64Len); 
        ogs_base64_encode(eap_base64,  eap_bytes, eap_len); 
        ogs_info("String Base64: [%s]", eap_base64);
        ogs_info("len string base64: [%lu]", strlen(eap_base64));
        // //EAP_SESSION.eap_payload = eap_base64;
        UeAuthenticationCtx.eap_payload = eap_base64;
    }

    //TODO how to make link for eap
