        ...
        amf_ue_t *amf_ue, void *data)
{
    ...
    size_t base64len = ogs_base64_encode_len(amf_ue->eap_message_len);
    //char eap_payload[base64len];
    char eap_payload[base64len];
    ...
    memset(&message, 0, sizeof(message));
    message.h.method = (char *)OGS_SBI_HTTP_METHOD_POST;
    message.h.uri = amf_ue->eap_session_url;

    eap_session = ogs_calloc(1, sizeof(*eap_session));
    if (!eap_session) {
        ogs_error("No eap_session");
        goto end;
    }

    
    ogs_trace("  EAP_MESSAGE - ");
    ogs_log_hexdump(OGS_LOG_TRACE, (void *)amf_ue->eap_message, amf_ue->eap_message_len);
    //ogs_hex_to_ascii(amf_ue->eap_message, amf_ue->eap_message_len, eap_payload, amf_ue->eap_message_len);
    ogs_base64_encode(eap_payload, amf_ue->eap_message, amf_ue->eap_message_len);
    eap_session->eap_payload = eap_payload;
    eap_session->auth_result = OpenAPI_auth_result_AUTHENTICATION_ONGOING;
    eap_session->supi = amf_ue->suci;
    // eap_session->k_seaf = NULL;
    // eap_session->_links = NULL;
    // eap_session->supported_features = NULL;

    message.EapSession = eap_session;
