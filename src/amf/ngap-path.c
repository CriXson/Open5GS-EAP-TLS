...
int ngap_open(void)
{
    ...
    ogs_list_for_each(&amf_self()->ngap_list, node) if (ngap_server(node) == NULL) return OGS_ERROR;

    ogs_list_for_each(&amf_self()->ngap_list6, node) if (ngap_server(node) == NULL) return OGS_ERROR;
