...
ogs_pkbuf_t *gmm_build_authentication_request_tls(amf_ue_t *amf_ue)
{
    ogs_nas_5gs_message_t message;
    ogs_nas_5gs_authentication_request_t *authentication_request =
        &message.gmm.authentication_request;
    ...
    authentication_request->eap_message.buffer = amf_ue->eap_message;
    authentication_request->eap_message.tls_start = 120;
    authentication_request->eap_message.length = amf_ue->eap_message_len;
    
    ogs_trace("  EAP_MESSAGE - ");
    ogs_log_hexdump(OGS_LOG_TRACE, amf_ue->eap_message, 6);
    ogs_trace("  EAP_MESSAGE - ");
    ogs_log_hexdump(OGS_LOG_TRACE, authentication_request->eap_message.buffer, 6);
    return ogs_nas_5gs_plain_encode(&message);
}
