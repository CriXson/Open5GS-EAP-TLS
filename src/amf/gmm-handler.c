                                                    ...
                                                    ogs_nas_5gs_registration_request_t *registration_request)
{
        ...
        OGS_NAS_5GS_REGISTRATION_REQUEST_5GMM_CAPABILITY_PRESENT)
    {
        ...
        amf_ue->gmm_capability.lte_positioning_protocol_capability = gmm_capability->lte_positioning_protocol_capability;
                                       ...
                                       ogs_nas_5gs_authentication_response_t *authentication_response)
{
    ...
    ogs_nas_eap_message_t authentication_eap_message;

    ogs_assert(amf_ue);
    ogs_assert(authentication_response);

    authentication_response_parameter = &authentication_response->authentication_response_parameter;
    ...
    {
       authentication_eap_message = authentication_response->eap_message;
       
        ogs_trace("  EAP_MESSAGE - ");
        ogs_log_hexdump(OGS_LOG_TRACE, (void *)authentication_response->eap_message.buffer, authentication_response->eap_message.length);
        if(authentication_eap_message.length > 1503 || authentication_eap_message.length < 5) // excluded the nas header 
        {
             ogs_error("[%s] Invalid length [%d]",
                  amf_ue->suci, authentication_eap_message.length);
        return OGS_ERROR;
        }
        //memcpy(authentication_response->eap_message.buffer, amf_ue->eap_message, OGS_NAS_MAX_EAP_MESSGE_LEN);
        amf_ue->eap_message = authentication_eap_message.buffer;
        amf_ue->eap_message_len = authentication_eap_message.length;
        ogs_assert(true ==
                   amf_ue_sbi_discover_and_send(
                       OGS_SBI_SERVICE_TYPE_NAUSF_AUTH, NULL,
                       amf_nausf_auth_build_authenticate_eap_session, amf_ue, NULL));
    ...
    if (strcmp(amf_ue->suci, "suci-0-001-01-0000-0-0-0000000001") == 0)
    {
        ogs_info("----------------------------TEST-----------------------------");
    }
                                                       ...
                                                       ogs_nas_5gs_security_mode_complete_t *security_mode_complete)
{
    ...
    if ((security_mode_complete->presencemask &
         OGS_NAS_5GS_SECURITY_MODE_COMPLETE_NAS_MESSAGE_CONTAINER_PRESENT) == 0)
    {
